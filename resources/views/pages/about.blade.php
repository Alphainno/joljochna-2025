...
@section('content')
  ...
  <div class="row align-items-stretch bg-light p-4 rounded-4 shadow-sm">
    ...
    <div class="col-lg-6 d-flex flex-column justify-content-center text-start" style="max-height: 500px; overflow-y: auto;">
      ...
      <h3 id="founderName" class="fw-semibold text-dark mb-1">মারুফ সাত্তার আলী</h3>
      <p id="founderPosition" class="text-primary fs-6 mb-3">প্রতিষ্ঠাতা, জলজোছনা</p>

      <p id="founderContent1" class="fs-6 lh-lg text-secondary mb-3">
      ...
      <p id="founderContent2" class="fs-6 lh-lg text-secondary mb-3">
      ...
      <p id="founderContent3" class="fs-6 lh-lg text-secondary mb-0">
...
<section class="container my-5">
  <div class="row align-items-stretch bg-light p-4 rounded-4 shadow-sm flex-lg-row-reverse">
    ...
    <div class="col-lg-6 d-flex flex-column justify-content-center text-start" style="max-height: 500px; overflow-y: auto;">
      ...
      <p id="chairmanContent" class="fs-6 lh-lg text-secondary mb-3">
      ...
      <p id="chairmanContent2" class="fs-6 lh-lg text-secondary mb-3">
      ...
      <p id="chairmanContent3" class="fs-6 lh-lg text-secondary mb-4">
        তিনি সর্বদা সমাজের প্রান্তিক মানুষের পাশে থাকার প্রতিশ্রুতি দেন এবং বিশ্বাস করেন যে উন্নয়নের মূল হলো মানবিকতা ও সততা।
      </p>

      <h4 id="chairmanName" class="fw-semibold text-dark mb-1">আব্দুল করিম</h4>
        ...
        (function(){
            ...
            async function loadAboutContent() {
                try {
                    ...
                    const founderResponse = await fetch('/api/about-sections?section_key=founder');
                    if (founderResponse.ok) {
                        const founder = await founderResponse.json();
                        if (founder) {
                            updateText('founderName', founder.name);
                            updateText('founderPosition', founder.position);
                            updateText('founderContent1', founder.content);
                            updateText('founderContent2', founder.content_2);
                            updateText('founderContent3', founder.content_3);
                            updateImage('founderImage', founder.image_url);
                        }
                    }

                    // Load Chairman Section
                    const chairmanResponse = await fetch('/api/about-sections?section_key=chairman');
                    if (chairmanResponse.ok) {
                        const chairman = await chairmanResponse.json();
                        if (chairman) {
                            updateText('chairmanName', chairman.name);
                            updateText('chairmanPosition', chairman.position);
                            updateText('chairmanContent', chairman.content);
                            updateText('chairmanContent2', chairman.content_2);
                            updateText('chairmanContent3', chairman.content_3);
                            updateImage('chairmanImage', chairman.image_url);
                        }
                    }
